...
import { YT_STAGES } from "@/lib/youtube/stages";

// ── Model definitions (mirrors the Micro Content sidebar) ─────────────────────
const MODELS = [
...
// ── Stage row — each dot uses the stage's own vibrant colour ──────────────────
...
// ── Main panel ────────────────────────────────────────────────────────────────
export default function YoutubeLeftPanel({
  currentStage,
  approvedStages = [],
  onGoToStage,
  demoMode = false,
  ...
  function getStatus(id) {
    if (approvedStages.includes(id)) return "approved";
    if (id === currentStage)         return "active";
    const max = approvedStages.length ? Math.max(...approvedStages) : 0;
    return id <= max + 1 ? "available" : "locked";
  }

  const progress = approvedStages.length / YT_STAGES.length;
  ...
  return (
    <aside className="hidden w-[200px] shrink-0 flex-col border-r border-[rgb(var(--border))] bg-[rgb(var(--bg-soft))] lg:flex">
      ...
      <div className="border-b border-[rgb(var(--border))] px-3.5 pt-3.5 pb-3">
        <p className="font-display text-sm font-bold text-[rgb(var(--text))]">Long Content Builder</p>
        <p className="text-[10px] text-faint">7-Stage YouTube Pipeline</p>
        <div className="mt-2.5 flex items-center justify-between">
          <span className="text-[9px] font-bold uppercase tracking-wider text-faint">
            {approvedStages.length}/{YT_STAGES.length} stages complete
          </span>
          <span className="text-[9px] font-bold text-[rgb(var(--accent))]">{Math.round(progress * 100)}%</span>
        </div>
        <div className="mt-1.5 h-1 w-full overflow-hidden rounded-full bg-[rgb(var(--border))]">
      ...
      <div className="space-y-2 border-t border-[rgb(var(--border))] px-3 pt-3 pb-14">
        <p className="text-[9px] font-bold uppercase tracking-[0.13em] text-faint">
          ...
          {MODELS.map((m) => {
            ...
            return (
              <button
                ...
                disabled={isDisabled}
                title={tooltip}
                className={`w-full rounded-lg border px-2.5 py-1.5 text-left text-[11px] font-semibold transition-all ${
                  isDisabled
